...
| `plan/INBOX/splice-env-locals.md` | PARTIAL (2026-09-06) | `##` splice blocks now see nelua-scope locals via `__nelua_scope` + positional injection. Verified: `## if v.type.is_cfloat then` works on both compilers, positional visibility matches, harness 0 regressions / 223 MATCH. **Verification (hash.nelua compiles) NOT met** -- two independent blockers: (1) `#|name|#` name splice (`plan/INBOX/preprocess-name-splice.md`); (2) **user-typed symbols** -- a param/local whose type is a user type (e.g. `a: R` for `R = @record{...}`) is never injected because `declaredTypeOf` only resolves builtins and the injection is gated on `sym.typ != nil` (`preprocessor.nim:2037-2040`). The oracle solves this by running the preprocessor interleaved with analysis (live `context.scope.symbols` lookup, `ppcontext.lua:48-49`); Nelu runs preprocessing once before analysis, so it cannot see resolved user types. Architectural, not a one-line fix. |
...
| `plan/DONE/typed-record-literal-cast.md` | CLOSED | The cast form of the record/union/enum constructor `(@T){ field = expr, ... }` was lowered against an anonymous `nlrec0` record, so codegen emitted `(struct Point)((struct nlrec0){...})` and gcc rejected it (`'struct nlrec0' has no member named 'x'`). **Fixed 2026-09-06**: `src/analyzer.nim` now analyzes the cast-form InitList against `castTarget` and flags the call as a constructor (the bare `T{...}` form already did this); `src/cgen.nim`'s C4 compound-literal path now picks `union` for union targets instead of always `struct` (gcc rejected `((struct nlrec2){...})`). Verified: `exam/cast_record.nelua` (named, mixed-order, union, bare) all MATCH the oracle; harness 247 MATCH / 0 regressions. |
