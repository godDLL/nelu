...
- **Corpus agents (2, DONE & integrated)** - `examples/fuzz/` (51 algorithms, oracle-verified, verdicts 22 MATCH / 5 DIFF / 21 CRASH / 2 HANG) and `examples/nelu/` (53 beyond-oracle examples, each verified oracle-rejects / ours-accepts). Both are devil harnesses: the breakage they find is the work queue. **`-ddx` devil (DONE & integrated 2026-09-03)** - classified all 220 `examples/*/**` tests (MATCH 129, O-REJ 47, DIFF-fail 8, OUR-REJ 30, SKIP 3) and renamed the 154 passing ones to `-ddx`; report `plan/devil-ddx-corpus.md`. Its headline finding: **the stdlib is entirely unreachable -- 51/51 `lib/` modules fail through ours**, root cause the `##` splice `in` keyword in `xoshiro256` plus a cascade; this is the single largest takeover blocker and drives the "Stdlib reachability" queue in `NOTE_backlog.md`. **`-ffs` devil (DONE & integrated 2026-09-03)** - the inverse of `-ddx`: classified all 220 files (ACCEPT 138, REJECT 47, SKIP 3, plus 2 design-time runtime aborts) and renamed the 25 clean oracle-rejects to `-ffs` and the 22 overlap ones (already `-ddx`) to `-ddx-ffs` -- both sets content-identical, integrated into live `examples/`; report `plan/ffs-corpus.md`. **CLI flag conformance (DONE 2026-09-03)** - characterized all 45 oracle flags over 864 dump + 200 build/run runs; harness now committed at `plan/cli_conformance.py` (`fabea4e`), task queue in `NOTE_backlog.md`, two fix agents launched 2026-09-03 evening (flag registration on `src/cli.nim`; assembly line + release checks + stdin on `src/compile.nim`/`src/main.nim`). Top gaps: `-Y`/`--assembly` malformed gcc line, `--sanitize`/`-S` no-op, `-r`/`--release` doesn't disable runtime checks, stdin `-` unsupported, no `lua` backend. It also corrected `plan/missing-cli-flags.md`, which was stale (most flags it lists as
missing are implemented and verified, including byte-identical `--config`); that doc was
rewritten 2026-09-03 to defer here as the single source of truth.
...
- **In flight (2026-09-03, late):** **splice B/C/D atomic** (`tmp/2026-09-03-2345-splice-bcd/`) - the core of the stdlib takeover blocker: preprocessor ordering (splices evaluated during the walk while `##` lines run after) plus a `scope` field for `PreprocessContext` plus an analyzer block pre-pass. **`print(nil)` -> `nil`** - **DONE & committed `d02799d`**: one-line `nelua_print_nil` fix in `src/cgen.nim` (`(null)` -> `nil`); `print(nil)`/`print(nilptr)`/`print(nil var)` all MATCH the oracle byte-for-byte, gates unchanged (cmp 39/1, regress M2 14/14 + M1 25/3/0 GREEN, examples 2/5/3).
