...
The `src/` tree is clean at `110630f`; everything below is committed, not uncommitted. (The `examples/fuzz/` and `examples/nelu/` corpora, plus `DEVIL.md`, are uncommitted work-in-progress from the corpus agents.) Our binary reports **v0.2.1** (`src/main.nim`).
...
- **Runtime per-TU inlining + libm removal (in flight)** - isolated copy at `tmp/2026-09-03-2005-runtime-inlining/`; design doc `plan/runtime-per-tu-inlining-design.md`. Emits only-referenced runtime helpers as `static` per-TU, drops the separate `src/runtime.c` link and `-lm`. See the design doc.
